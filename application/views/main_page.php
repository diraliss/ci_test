...
<div id="app">
        ...
        <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
            ...
            <div class="collapse navbar-collapse" id="navbarTogglerDemo01">
                ...
                <? if (User_model::is_logged()) { ?>
                    ...
                    <li class="nav-item">
                        <button type="button" class="btn btn-info" @click="openUsersTransactions">Your transactions</button>
                    </li>
                    <li class="nav-item">
                        <button type="button" class="btn btn-info" @click="openUsersBoosterpacks">Your boosterpacks</button>
                    </li>
                    <li class="nav-item">
                        <button type="button" class="btn btn-info" @click="openUsersInfo">Your info</button>
                    </li>
                <? } ?>
            </div>
            <? if (User_model::is_logged()) { ?>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="card mb-3">
                        ...
                        <div class="card-body">
                            <div class="likes" @click="addPostLike(post.id)">
                            ...
                            <div class="card-text" v-for="comment in post.comments">
                                {{comment.id}} {{comment.user.personaname + ' - '}} <small>{{comment.text}}</small>
                                <span class="likes" @click="addCommentLike(comment.id)">
                                    <span class="heart-wrap d-inline-block ml-3">
                                        <span class="heart">
                                            <svg class="bi bi-heart" width="1em" height="1em" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                                <path fill-rule="evenodd" d="M8 2.748l-.717-.737C5.6.281 2.514.878 1.4 3.053c-.523 1.023-.641 2.5.314 4.385.92 1.815 2.834 3.989 6.286 6.357 3.452-2.368 5.365-4.542 6.286-6.357.955-1.886.838-3.362.314-4.385C13.486.878 10.4.28 8.717 2.01L8 2.748zM8 15C-7.333 4.868 3.279-3.04 7.824 1.143c.06.055.119.112.176.171a3.12 3.12 0 01.176-.17C12.72-3.042 23.333 4.867 8 15z" clip-rule="evenodd"/>
                                            </svg>
    ...
    <!-- Modal -->
    <div class="modal fade" id="backendModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="backendModalTitle">{{backendData.title}}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body" id="backendModalBody">
                    <table class="table" v-if='backendData.type="array"'>
                        <thead>
                            <tr>
                                <th v-for='(value, key) in backendData.data[0]'>{{key}}</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for='item in backendData.data'>
                                <td v-for='(value, key) in item'>{{value}}</td>
                            </tr>
                        </tbody>
                    </table>
                    <table class="table" v-if='backendData.type="single"'>
                        <tbody>
                            <tr v-for='(value, key) in backendData.singleData'>
                                <th>{{key}}</th>
